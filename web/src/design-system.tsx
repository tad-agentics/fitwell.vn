...
export interface PainScoreSelectorProps {
  ...
  promptTimeLabel?: string // e.g. 'sáng nay' for check-in, default 'hôm nay'
}
...
export const StepProgressBar = ({ total, current }: StepProgressBarProps) => (
  <div style={{ display: 'flex', gap: '4px' }}>
    {Array.from({ length: total }, (_, i) => (
      <div key={i} style={{ flex: 1, height: 3, borderRadius: radius.full, background: colors.bg3, overflow: 'hidden' }}>
        <div style={{
          height: '100%',
          width: i < current ? '100%' : '0%',
          borderRadius: radius.full,
          background: colors.t0,
          transition: 'width 700ms cubic-bezier(0.2,0,0.1,1) 400ms',
        }} />
      </div>
    ))}
  </div>
...
export const PainScoreSelector = ({ value, onChange, conditionName, promptTimeLabel = 'hôm nay' }: PainScoreSelectorProps) => (
  <div>
    {conditionName && (
      <div style={{ fontFamily: typography.fontPrimary, fontSize: typography.size14, fontWeight: typography.w600, color: colors.t0, marginBottom: '12px' }}>
        {conditionName} {promptTimeLabel} thế nào?
      </div>
    )}
