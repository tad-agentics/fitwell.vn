...
> **IBM Plex Mono for CTAs** (replaces DM Mono per TechUX v1.3) — DM Mono clips stacked Vietnamese diacritics (ớ, ế, ứ, ở) in uppercase; IBM Plex Mono renders them correctly. The mono-font-for-CTA principle is unchanged: precision-cut letterforms signal authority without decoration.
...
| Background | `#FFFFFF` — flat, no shadow (RL principle) |
| Top border | 1px `#EBEBF0` — only visual separator |
| Active tab | `#041E3A` icon + label — no background fill on the tab itself |
| Inactive tab | `#9D9FA3` icon + label |
| Label font | Be Vietnam Pro · 10px · inactive weight 400 · active weight 600 |
| Icon style | 24×24px · stroke-based · 1.5px stroke weight · no fill |
| Unread dot | 6px circle · `#D97706` amber · top-right of Tuần này tab · Sunday + Monday only |
| Min tap target | 48px per tab (WCAG AA) |
| Active transition | 150ms ease-out colour change — no slide, no bounce |

---

## 07 — Imagery Principles
...
- IBM Plex Mono · weights 400, 500 · ~24kb
- **Total: ~122kb** — acceptable on 4G; test on 3G to confirm <2s load
